...
_Progress note: startup receiver bootstrap now preserves saved DMX universe/start-channel values by applying PlayerPrefs-backed address settings without re-saving defaults during `NetworkingModeManager` initialization, with added debug logs for persistence tracing._
...
_Progress note: `MovingHeadBeamController` now re-initializes from static manager-ready events (`DmxModeManager` + `NetworkingModeManager`) so startup order races no longer require manual disable/enable cycles in Moving Head mode._
_Progress note: WebUI advanced-networking state now reads/writes the new DMX settings snapshot flow correctly (`isSAcnMode`, timeout seconds, and persisted sACN settings), and multicast IP is now read-only in browser clients._
_Progress note: WebUI settings synchronization now uses a runtime `serverSessionId` guard so stale browser tabs cannot POST old settings into a restarted app instance; client refresh/polling also marks disconnected state as stale and blocks autosave until re-synced._
