    ...
    async fetch(request, env) {
        ...
        try {
            ...
            if (cachedRaw) {
                ...
                // Add new deviceId if not already tracked
                if (!cached.deviceIds.includes(deviceId)) {
                    cached.deviceIds.push(deviceId);
                    await env.PURCHASE_CACHE.put(
                        cacheKey,
                        JSON.stringify(cached),
                        { expirationTtl: 86400 } // cache 24h
                    );
                }
            ...
            if (!googleRes.ok) {
                return json({ productId, valid: false, revoked: false });
            }
            ...
            return json({ valid: false, revoked: false });
        }
    }
};

// Get OAuth access token for Google Play
